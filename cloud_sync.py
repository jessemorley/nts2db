...
# AND create the inventory table:
# CREATE TABLE dropbox_inventory (id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, name TEXT UNIQUE, created_at TIMESTAMP WITH TIME ZONE DEFAULT now());
# ---------------------------------------
...
def update_dropbox_inventory(dbx):
    ...
    print("🔍 Updating Dropbox inventory...")
    try:
        files = []
        ...
        # Clear old inventory (simple method)
        requests.delete(f"{SUPABASE_URL}/rest/v1/dropbox_inventory?id=gt.0", headers=headers)
        # Bulk insert new inventory (limit 100 for safety)
        payload = [{"name": f} for f in files[:100]]
        if payload:
            requests.post(f"{SUPABASE_URL}/rest/v1/dropbox_inventory", headers=headers, json=payload)
        print(f"✅ Inventory updated: {len(files)} files.")
    except Exception as e:
        print(f"⚠️ Inventory update error: {e}")

def get_dbx_client():
    return dropbox.Dropbox(oauth2_refresh_token=DBX_REFRESH_TOKEN, app_key=DBX_APP_KEY, app_secret=DBX_APP_SECRET)
...
def fetch_liked_tracks():
    ...
    ydl_opts = {'extract_flat': True, 'quiet': True, 'no_warnings': True, 'playlist_items': '1,10'}
    try:
...
def sync_to_dropbox():
    ...
    for i, item in enumerate(items):
        ...
        try:
            ...
            ydl_opts = {'outtmpl': 'temp_track.mp3', 'format': 'bestaudio/best', 'quiet': True, 'no_warnings': True, 'progress_hooks': [progress_hook]}
            try:
